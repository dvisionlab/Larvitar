...
export function addMouseKeyHandlers(config){
    ...
    function onKeyDown(evt){
      // keyboard shortcuts
      let codes = Object.keys(config.keyboard_shortcuts).map(key => KEY_CODES[key])
      if (codes.includes(evt.keyCode) && evt.altKey){
        let key = Object.keys(config.keyboard_shortcuts).filter(key => KEY_CODES[key] == evt.keyCode).pop();
        if (config.debug) console.log('active', config.keyboard_shortcuts[key])
        larvitar.setToolActive(config.keyboard_shortcuts[key])
        document.addEventListener('keydown', onKeyDown, {once: true});
      }
      // right drag + shift
      else if (evt.keyCode == KEY_CODES["shift"]){ 
        if (config.debug) console.log('active', config.mouse_button_right.shift)
        larvitar.setToolActive(config.mouse_button_right.shift, {mouseButtonMask:2})
      ...
      // right drag + ctrl
        ...
        larvitar.setToolActive(config.mouse_button_right.ctrl, {mouseButtonMask:2})
        document.addEventListener('keyup', onKeyUp, {once: true});
      }
      // restore default
      else{
    ...
    function onKeyUp(e){
      ...
      larvitar.setToolActive(config.mouse_button_right.default, {mouseButtonMask:2})
      document.addEventListener('keydown', onKeyDown, {once: true});
    }
    
    larvitar.setToolActive(config.mouse_button_right.default, {mouseButtonMask:[1,2]})
    document.addEventListener('keydown', onKeyDown, {once: true});
  }