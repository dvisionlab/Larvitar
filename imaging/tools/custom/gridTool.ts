...
export class GridTool extends BaseTool {
  ...
  triggerDrawGrid(enabledElement: EnabledElement) {
    ...
    let imageMetadata;
    if (imageManager) {
      for (const [seriesId, seriesObj] of Object.entries(imageManager)) {
        if (seriesObj.imageIds && seriesObj.imageIds.includes(image.imageId)) {
          imageMetadata = seriesObj.instances[image.imageId].metadata;
        }
      }
    }
