...
export class LivewireScissors {
  ...
  public startSearch(startPoint: [number, number]): void {
   ...
   */
  public findMinNearby(testPoint: [number, number], delta = 2) {
    const [x, y] = testPoint;
    const { costs } = this;
   ...
   */
  public findPathToPoint(targetPoint: [number, number]): [number, number][] {
    if (!this.startPoint) {
   ...
   * @returns Point coordinate (x,y)
   */
  private _getPointCoordinate = (index: number): [number, number] => {
  ...
  public getCost(pointA: [number, number], pointB: [number, number]): number {
   ...
   */
  private _getNeighborPoints(point: [number, number]): [number, number][] {
    const { width, height } = this;
  ...
  public static createInstanceFromRawPixelData(
    ...
    return new LivewireScissors(grayscalePixelData as any, width, height);
  }
}
